...
sys_chmod(void)
{
  ...
  if(ip->permission == 5){
    // Si el archivo es inmutable, no se pueden cambiar los permisos
    iunlockput(ip);
    end_op();
    return -1;
  }
  ...
  ip->permission = mode;
  iupdate(ip); // Aunque no modificamos el dinode, aseguramos consistencia
  iunlockput(ip);
  end_op();
  return 0;
}
